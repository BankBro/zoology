...
- 简洁实用: 以简洁实用为主, 不要过度设计. 以当前任务和已经验证的需求为边界, 优先采用最小、直接、可维护的实现; 不为未经确认的未来需求预先增加抽象层、兼容层、配置项或复杂机制.
...
- 独立实验 worktree 进入终态后, 若实验确认有效且相关改动需要进入主仓主要开发分支, 必须先将有效提交合入并推送到该分支, 再把 worktree 产生的未被 Git 跟踪的实验产物迁入主仓对应实验目录. 清理 worktree 前必须核对提交、工作区、产物路径、文件数量或大小以及必要 hash, 确认源码和产物均已安全转移且可追溯; 不得先删除 worktree 再补做收敛.
