...
- GDN dtype 记录: GatedDeltaNet 的 FLA kernel dtype 可通过 `GDN_KERNEL_DTYPE=auto|input|float32|float16|bfloat16` 控制. 默认 `auto` 保持现有行为, CUDA sm80+ 使用 bf16, sm80 以下使用 fp16, CPU 使用 fp32; `input` 表示不做 GDN 内部 kernel dtype cast. 任何非默认 dtype 实验都必须在 artifact/report/summary 中记录该字段, 并注明它是 kernel dtype 口径而不是全模型 dtype.
