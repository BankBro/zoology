...
结论: stage3 local-only checkpoint 在 official longer-MQAR 分布上接近随机, 说明只有 exact local branch 不能解决远距离任务. local1 能覆盖一部分 1024x256, 但长度扩展到 4096x1024 后明显崩掉. local4 的 best/last 差异很大, `best.pt` 在所有 longer slices 上显著强于 `last.pt`, 因此 local4 需要同时报告 best 和 last, 不能只看 final.
...
结论: local4 best 的优势在 `1025-2048` 和 `2049-4096` 仍然存在, 这不是 64-token exact local window 能直接解释的距离范围. 这支持阶段 1/2 的判断: Flash 的长距离优势不只是 local exact attention.

## Near-distance enriched eval

near-distance enriched 使用 position metadata 计算 `query_pos - value_pos`. 每个样本轮换强化一个近距离桶, `<=32` 固定代表距离 31, `33-64` 在 odd distances 33-63 内均匀采样, `65-128` 在 odd distances 65-127 内均匀采样.
...
1. local-only 在 `<=32` 为 1.0, 但在 `65-128` 接近 0, 说明 eval 确实捕捉到了 exact local branch 的边界收益.
2. local-only 在 `33-64` 只有约 0.48-0.50, 说明“64 token local window”不是一个对任意 query 都保证完整覆盖的平滑窗口. block alignment 和 causal/local block 实现会让 33-64 成为边界区域, 而 `<=32` 才是更稳的 exact local 区间.
3. local4 last 在 near buckets 上很强, 但 official 4096x1024 很弱. 这说明 last checkpoint 更偏近距离/局部能力, 长距离能力退化.
4. local4 best 在 official longer-MQAR 上强, 但 near `<=64` 很弱, 尤其 4096x1024 的 `<=32` 和 `33-64` 近乎 0. 这反而说明 local4 best 的 longer-MQAR 优势主要不是靠 exact local branch, 更像 remote/VQ/GD residual 路径贡献.
...
但这次补强后, 更强的结论是: Flash 在长距离上的优势不能主要归因于 64-token local exact attention. local-only 只能解决很近距离, local4 best 在 1025+ 和 2049+ bucket 仍强, 而 near `<=64` 并不强. 这说明 remote/VQ/GD residual 机制仍是长距离收益的必要解释.

## 是否还需要 GDN+exact-local 阶段 4

建议仍保留阶段 4, 但优先级可以低于当前报告收尾. 原因是当前 evidence 已经足够说明 Flash 长距离优势不只是 local window, 但如果要做最严格的 model-class fairness, 仍应给 GDN 增加同等 exact-local branch 后重跑关键 eval, 以消除 reviewer 对 baseline local capacity 的质疑.
