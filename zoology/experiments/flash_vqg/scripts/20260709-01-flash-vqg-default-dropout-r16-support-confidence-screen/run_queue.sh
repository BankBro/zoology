...
TIMESTAMP="${SUPPORT_CONFIDENCE_TIMESTAMP:-$(date -u +%Y%m%dT%H%M%SZ)}"
MODE="${MODE:-formal}"
QUEUE_NAME="${1:-}"
shift || true
...
if [[ -z "${QUEUE_NAME}" ]]; then
  cat >&2 <<'USAGE'
Usage:
  run_queue.sh <support-2080ti-gpu1|support-3090-gpu0> [targets...]

If targets are omitted, all 8 support-confidence targets run in order.
USAGE
  exit 2
fi
...
valid_target() {
  local target="$1"
  case "${target}" in
    s125-baseline-r16-joint|s125-read-gate-r16|s125-read-softmargin-r16|s125-read-gate-softmargin-r16|\
s124-baseline-r16-joint|s124-read-gate-r16|s124-read-softmargin-r16|s124-read-gate-softmargin-r16)
      return 0
      ;;
    *)
      return 1
...
OUTPUT_ROOT="${OUTPUT_ROOT:-${SCRIPT_DIR}/outputs/${QUEUE_NAME}-${TIMESTAMP}-${MODE}}"
...
run_one() {
  ...
  "${PYTHON_BIN}" "${SCRIPT_DIR}/support_confidence_screen.py" cache-hash \
    --machine-name "${MACHINE_NAME}" \
    --target "${target}" \
    --variant "${target}" \
    --output-json "${OUTPUT_ROOT}/cache-hash-${target}.json"

  "${PYTHON_BIN}" "${SCRIPT_DIR}/support_confidence_screen.py" preflight \
    --machine-name "${MACHINE_NAME}" \
    --target "${target}" \
    --variant "${target}" \
  ...
  (
    ...
    "${PYTHON_BIN}" "${SCRIPT_DIR}/support_confidence_screen.py" train \
      --machine-name "${MACHINE_NAME}" \
      --target "${target}" \
      --variant "${target}" \
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/support_confidence_screen.py" verify-init \
  --machine-name "${MACHINE_NAME}" \
...
printf "%q " bash "${SCRIPT_DIR}/run_queue.sh" "${QUEUE_NAME}" "${TARGETS[@]}" > "${OUTPUT_ROOT}/command.txt"
