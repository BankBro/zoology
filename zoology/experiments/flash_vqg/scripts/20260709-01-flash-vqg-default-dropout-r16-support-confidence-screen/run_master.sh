...
launch_remote_3090_gpu0() {
  local queue="support-3090-gpu0"
  local gpu="0"
  local output_root="${SCRIPT_DIR}/outputs/${queue}-${TIMESTAMP}-${MODE}"
  local log_path="${MASTER_ROOT}/logs/${queue}.supervisor.log"
  local remote_cmd
  remote_cmd=$(
    printf "cd %q && SUPPORT_CONFIDENCE_TIMESTAMP=%q MODE=%q CONTINUE_ON_FAIL=%q POLL_SECONDS=%q SMOKE_TRAIN_STEPS=%q SMOKE_VALIDATION_BATCHES=%q MAX_TRAIN_STEPS=%q OUTPUT_ROOT=%q GPU=%q bash %q %q" \
