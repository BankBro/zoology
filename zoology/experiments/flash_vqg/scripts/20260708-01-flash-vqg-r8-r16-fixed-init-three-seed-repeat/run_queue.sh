...
if [[ -z "${QUEUE_NAME}" ]]; then
  cat >&2 <<'USAGE'
Usage:
  run_queue.sh <fixed-2080ti-gpu1|fixed-3090-gpu0> [targets...]

If targets are omitted, all 12 fixed-init seed/read/repeat targets run in order.
USAGE
  exit 2
fi
...
run_one() {
  ...
  "${PYTHON_BIN}" "${SCRIPT_DIR}/fixed_init_three_seed_repeat.py" cache-hash \
    --machine-name "${MACHINE_NAME}" \
    --target "${target}" \
    --variant "${target}" \
    --output-json "${OUTPUT_ROOT}/cache-hash-${target}.json"

  "${PYTHON_BIN}" "${SCRIPT_DIR}/fixed_init_three_seed_repeat.py" preflight \
    --machine-name "${MACHINE_NAME}" \
    --target "${target}" \
    --variant "${target}" \
  ...
  (
    ...
    "${PYTHON_BIN}" "${SCRIPT_DIR}/fixed_init_three_seed_repeat.py" train \
      --machine-name "${MACHINE_NAME}" \
      --target "${target}" \
      --variant "${target}" \
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/fixed_init_three_seed_repeat.py" verify-init \
  --machine-name "${MACHINE_NAME}" \
