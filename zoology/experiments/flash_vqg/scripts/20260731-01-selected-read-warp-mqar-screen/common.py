...
EXPECTED_FLASH_COMMIT = "efc75ad5539b636b026c76bedb70878bfe2390cf"
...
SEEDS = (123,)
BASELINE = "s1-head8192"
BACKENDS = {
    BASELINE: "triton_deterministic_s1_head",
    "r1a-owner-w2": "triton_state_owner_r1a_s1_w2",
    "r1b-preproject-w2": "triton_state_owner_r1b_preproject_w2_fast",
}
VARIANTS = tuple(BACKENDS)
FORMAL_ORDER = tuple((variant, 123) for variant in VARIANTS)
LONGER_CASES = (
...
EXTRAPOLATION_DELTA_MIN = -0.02


def run_tag() -> str:
    value = os.environ.get("MQAR_SELECTED_WARP_RUN_TAG", "").strip()
    if not re.fullmatch(r"[A-Za-z0-9_.-]+", value):
        raise RuntimeError("MQAR_SELECTED_WARP_RUN_TAG must be a non-empty safe name.")
    return value
...
def descriptor(variant: str, seed: int) -> dict[str, Any]:
    if variant not in BACKENDS or seed not in SEEDS:
