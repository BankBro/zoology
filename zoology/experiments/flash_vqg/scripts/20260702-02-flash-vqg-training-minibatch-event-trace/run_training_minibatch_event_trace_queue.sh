...
if [[ "$#" -lt 1 ]]; then
  cat >&2 <<'USAGE'
Usage:
  run_training_minibatch_event_trace_queue.sh <queue-name>

Queues:
  inline-2080ti-gpu0-baseline
  inline-2080ti-gpu1-ucap0p5
  inline-3090-gpu0-all
  inline-3090-gpu0-baseline
  inline-3090-gpu0-ucap0p5
USAGE
  exit 2
fi
...
TIMESTAMP="${TRAIN_INLINE_EVENT_TRACE_TIMESTAMP:-$(date -u +%Y%m%dT%H%M%SZ)}"
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/training_minibatch_event_trace.py" verify-init \
...
for target in "${TARGETS[@]}"; do
  ...
  "${PYTHON_BIN}" "${SCRIPT_DIR}/training_minibatch_event_trace.py" cache-hash \
    --machine-name "${MACHINE_NAME}" \
    --target "${target}" \
    --variant "${variant}" \
    --output-json "${OUTPUT_ROOT}/cache-hash-${target}.json"

  "${PYTHON_BIN}" "${SCRIPT_DIR}/training_minibatch_event_trace.py" preflight \
    --machine-name "${MACHINE_NAME}" \
    --target "${target}" \
    --variant "${variant}" \
  ...
  (
    ...
    "${PYTHON_BIN}" "${SCRIPT_DIR}/training_minibatch_event_trace.py" train \
      --machine-name "${MACHINE_NAME}" \
      --target "${target}" \
      --variant "${variant}" \
  ...
  (
    export CUDA_VISIBLE_DEVICES="${GPU}"
    "${PYTHON_BIN}" "${SCRIPT_DIR}/training_minibatch_event_trace.py" hash-probe \
      --machine-name "${MACHINE_NAME}" \
      --target "${target}" \
      --init-checkpoint "${INIT_CHECKPOINT}" \
