...
LAUNCH_ID_PREFIX="${LAUNCH_ID_PREFIX:-flash-vqg-clr-r8-den1}"
...
LOG_DIR="${LOG_DIR:-${ROOT_DIR}/tmp/flash_vqg_clr_r8_den1}"

mkdir -p "${LOG_DIR}"

echo "==> clr_v1-r8-den1 使用 GPU ${GPU_ID}, launch_id_prefix=${LAUNCH_ID_PREFIX}"
echo "==> 日志目录: ${LOG_DIR}"
...
printf '%q ' "${CMD[@]}"
printf '\n'
"${CMD[@]}" | tee "${LOG_DIR}/clr_v1_r8_den1.log"

echo "==> 实验结束"
echo "==> 日志: ${LOG_DIR}/clr_v1_r8_den1.log"
