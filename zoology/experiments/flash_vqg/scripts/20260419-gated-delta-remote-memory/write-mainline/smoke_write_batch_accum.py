...
_SMOKE_COMMON = _load_module(
    "flash_vqg_gd_write_smoke_common",
    Path(__file__).resolve().parents[2] / "20260402-clr-v1-mainline" / "e2-remote-interface" / "smoke_common.py",
)
...
def _load_builder():
    module = _load_module(
        "flash_vqg_gd_write_builder",
        Path(__file__).resolve().parent / "config_builder.py",
    )
    return module.build_write_smoke_configs
...
def main() -> None:
    ...
    parser.add_argument("--launch-id-prefix", type=str, default="flash-vqg-gd-write-smoke")
    ...
    parser.add_argument("--env-out", type=str, default=None)
    ...
    for train_batch_size, eval_batch_size, gradient_accumulation_steps in CANDIDATE_BATCH_COMBOS:
        print(
            "[smoke] gated-delta-write "
            f"tbs={train_batch_size} ebs={eval_batch_size} ga={gradient_accumulation_steps}",
            flush=True,
        )
        args = _build_args(
            namespace,
    ...
    if namespace.summary_out is not None:
        Path(namespace.summary_out).write_text(summary_path.read_text(encoding="utf-8"), encoding="utf-8")
        summary_path = Path(namespace.summary_out)
    if namespace.env_out is not None:
        Path(namespace.env_out).write_text(env_path.read_text(encoding="utf-8"), encoding="utf-8")
        env_path = Path(namespace.env_out)

    print(f"SMOKE_SUMMARY_JSON={summary_path}")
    print(f"SMOKE_ENV_FILE={env_path}")
    if chosen is None:
        raise SystemExit("No batch/GA candidate passed gated-delta write smoke.")
