...
METRICS_WHITE_LIST_FILE_DIAG="${METRICS_WHITE_LIST_FILE_DIAG:-${SCRIPT_DIR}/metrics.yaml}"
WRITE_ENV_FILE="${WRITE_ENV_FILE:-${MAINLINE_DIR}/write-mainline/write_smoke.env}"
...
if [[ -z "${TRAIN_BATCH_SIZE}" || -z "${EVAL_BATCH_SIZE}" || -z "${GRADIENT_ACCUMULATION_STEPS}" ]]; then
  echo "需要先有 write smoke 结果, 再起 read diagnostics." >&2
  exit 1
fi

BUILDER_SPEC="${SCRIPT_DIR}/config_builder.py:build_read_diagnostics_configs"
...
then
  "${PYTHON_BIN}" -m zoology.experiments.flash_vqg.run_flash_vqg_suite \
    ...
    --cache-dir "${CACHE_DIR}" \
    --metrics-white-list-file "${METRICS_WHITE_LIST_FILE_DIAG}" \
    --project "${PROJECT}" \
    --entity "${ENTITY}" \
    --launch-id-prefix "${LAUNCH_ID_PREFIX_DIAG}" \
