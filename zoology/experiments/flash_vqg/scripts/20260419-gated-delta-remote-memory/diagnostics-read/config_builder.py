...
DIAG_SPECS = (
    ("diag-score-only-shared-local-den", "score_only", "shared_local_den"),
    ("diag-score-only-residual-add", "score_only", "residual_add"),
)
...
def _common_builder_kwargs(args, *, selector_mode: str, merge_mode: str):
    ...
    return (
        dict(
            ...
            fox_clr_residual_update_mode="delta",
            fox_clr_residual_forget_mode="code_aware",
            ...
            experiment_part="gated_delta_read_diag",
            experiment_mode=f"{selector_mode}_{merge_mode}",
...
def build_read_diagnostics_configs(args):
    configs = []
    for mode_name, selector_mode, merge_mode in DIAG_SPECS:
        ...
        if len(built) != 1:
            raise RuntimeError(f"Expected exactly 1 config for {mode_name}, got {len(built)}")
        run_id = f"dense-t025-gated-delta-{mode_name}-s{seed_value}-d{data_seed}"
        configs.append(_rewrite_run_id(built[0], run_id=run_id))
    return configs
