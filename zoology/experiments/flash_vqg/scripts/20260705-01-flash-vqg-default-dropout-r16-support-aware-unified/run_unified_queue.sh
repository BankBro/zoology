...
if [[ "$#" -lt 1 ]]; then
  cat >&2 <<'USAGE'
Usage:
  run_unified_queue.sh <queue-name>
...
TIMESTAMP="${R16_SUPPORT_UNIFIED_TIMESTAMP:-$(date -u +%Y%m%dT%H%M%SZ)}"
...
if [[ -n "${UNIFIED_TARGETS:-}" ]]; then
  read -r -a TARGETS <<< "${UNIFIED_TARGETS}"
fi
...
run_one() {
  ...
  "${PYTHON_BIN}" "${SCRIPT_DIR}/r16_support_aware_unified.py" cache-hash \
    --machine-name "${MACHINE_NAME}" \
    --target "${target}" \
    --variant "${variant}" \
    --output-json "${OUTPUT_ROOT}/cache-hash-${target}.json"

  "${PYTHON_BIN}" "${SCRIPT_DIR}/r16_support_aware_unified.py" preflight \
    --machine-name "${MACHINE_NAME}" \
    --target "${target}" \
    --variant "${variant}" \
  ...
  (
    ...
    "${PYTHON_BIN}" "${SCRIPT_DIR}/r16_support_aware_unified.py" train \
      --machine-name "${MACHINE_NAME}" \
      --target "${target}" \
      --variant "${variant}" \
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/r16_support_aware_unified.py" verify-init \
  --machine-name "${MACHINE_NAME}" \
