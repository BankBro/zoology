...
FORMAL_ORDER = (
    ("a0-fixed-off", 123),
    ("a1-fixed-post-phase1", 123),
    ("a0-fixed-off", 124),
    ("a1-fixed-post-phase1", 124),
    ("a0-fixed-off", 125),
    ("a1-fixed-post-phase1", 125),
)
LONGER_CASES = (
...
def run_tag() -> str:
    value = os.environ.get("MQAR_DETERMINISTIC_SELECTED_RUN_TAG", "").strip()
    if not re.fullmatch(r"[A-Za-z0-9_.-]+", value):
        raise RuntimeError(
            "MQAR_DETERMINISTIC_SELECTED_RUN_TAG must be a non-empty safe name."
        )
    return value
