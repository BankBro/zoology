...
DETERMINISM_SCRIPT = LOCAL_DIR / "determinism.py"
...
class Queue:
    ...
    def determinism(self) -> None:
        self.current_phase = "determinism"
        path = self.root / "determinism" / "summary.json"
        if not path.exists() or load_json(path).get("status") != "passed":
            self.run_process("determinism", [str(PYTHON), str(DETERMINISM_SCRIPT)])
        if load_json(path).get("status") != "passed":
            raise RuntimeError("Determinism gate did not pass.")
    ...
    def collect(self) -> None:
        ...
        if code == 2 or summary.get("status") != "passed":
            self.status(str(summary.get("status", "failed")), summary=summary)
            raise RuntimeError(
                "A1 failed the registered determinism or MQAR quality gate."
            )
...
def main() -> int:
    ...
    try:
        ...
        state = "failed"
        summary_path = queue.root / "final-summary" / "summary.json"
        if queue.current_phase == "collect" and summary_path.exists():
            state = str(load_json(summary_path).get("status", state))
        queue.status(
            state,
