...
if [[ "$#" -lt 1 ]]; then
  cat >&2 <<'USAGE'
Usage:
  run_injection_warmup_screen_queue.sh <queue-name>

Queues:
  injwarm-2080ti-gpu0-baseline-linear512
  injwarm-2080ti-gpu1-silent64linear512
  injwarm-3090-gpu0-all
  injwarm-3090-gpu0-baseline
  injwarm-3090-gpu0-linear512
  injwarm-3090-gpu0-silent64linear512
USAGE
  exit 2
fi
...
TIMESTAMP="${INJECTION_WARMUP_SCREEN_TIMESTAMP:-$(date -u +%Y%m%dT%H%M%SZ)}"
...
case "${QUEUE_NAME}" in
  injwarm-2080ti-gpu0-baseline-linear512)
    ...
    GPU="0"
    TARGETS=("baseline-r2" "inj-warmup-linear512-r2")
    ;;
  injwarm-2080ti-gpu1-silent64linear512)
    MACHINE_NAME="${MACHINE_NAME:-2080ti}"
    GPU="1"
    TARGETS=("inj-warmup-silent64-linear512-r2")
    ;;
  injwarm-3090-gpu0-all)
    MACHINE_NAME="${MACHINE_NAME:-3090}"
    GPU="0"
    TARGETS=("baseline-r2" "inj-warmup-linear512-r2" "inj-warmup-silent64-linear512-r2")
    ;;
  injwarm-3090-gpu0-baseline)
    MACHINE_NAME="${MACHINE_NAME:-3090}"
    GPU="0"
  ...
  injwarm-3090-gpu0-linear512)
    MACHINE_NAME="${MACHINE_NAME:-3090}"
    GPU="0"
    TARGETS=("inj-warmup-linear512-r2")
    ;;
  injwarm-3090-gpu0-silent64linear512)
    MACHINE_NAME="${MACHINE_NAME:-3090}"
    GPU="0"
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/injection_warmup_screen.py" verify-init \
...
for target in "${TARGETS[@]}"; do
  ...
  "${PYTHON_BIN}" "${SCRIPT_DIR}/injection_warmup_screen.py" cache-hash \
    --machine-name "${MACHINE_NAME}" \
    --target "${target}" \
    --variant "${variant}" \
    --output-json "${OUTPUT_ROOT}/cache-hash-${target}.json"

  "${PYTHON_BIN}" "${SCRIPT_DIR}/injection_warmup_screen.py" preflight \
    --machine-name "${MACHINE_NAME}" \
    --target "${target}" \
    --variant "${variant}" \
  ...
  (
    ...
    "${PYTHON_BIN}" "${SCRIPT_DIR}/injection_warmup_screen.py" train \
      --machine-name "${MACHINE_NAME}" \
      --target "${target}" \
      --variant "${variant}" \
  ...
  (
    export CUDA_VISIBLE_DEVICES="${GPU}"
    "${PYTHON_BIN}" "${SCRIPT_DIR}/injection_warmup_screen.py" hash-probe \
      --machine-name "${MACHINE_NAME}" \
      --target "${target}" \
      --init-checkpoint "${INIT_CHECKPOINT}" \
