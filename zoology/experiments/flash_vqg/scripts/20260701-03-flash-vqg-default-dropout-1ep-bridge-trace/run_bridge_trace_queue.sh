...
if [[ "$#" -lt 1 ]]; then
  cat >&2 <<'USAGE'
Usage:
  run_bridge_trace_queue.sh <queue-name>

Queues:
  bridge-2080ti-gpu0-r2
  bridge-2080ti-gpu1-r4
  bridge-2080ti-gpu0-drop005
  bridge-2080ti-gpu1-drop005
  bridge-3090-gpu0-all
  bridge-3090-gpu0-r2
  bridge-3090-gpu0-r4
  bridge-3090-gpu0-drop005
USAGE
  exit 2
fi
...
TIMESTAMP="${BRIDGE_TRACE_1EP_TIMESTAMP:-$(date -u +%Y%m%dT%H%M%SZ)}"
...
CAPTURE_STEPS="${CAPTURE_STEPS:-0,16,64,128,256,384,512,704}"
...
case "${QUEUE_NAME}" in
  ...
  bridge-3090-gpu0-all)
    ...
    GPU="0"
    TARGETS=("default-r2" "default-r4" "dropout005-r4")
    ;;
  bridge-3090-gpu0-r2)
    MACHINE_NAME="${MACHINE_NAME:-3090}"
    GPU="0"
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/bridge_trace_1ep.py" verify-init \
...
for target in "${TARGETS[@]}"; do
  ...
  "${PYTHON_BIN}" "${SCRIPT_DIR}/bridge_trace_1ep.py" cache-hash \
    --machine-name "${MACHINE_NAME}" \
    --target "${target}" \
    --variant "${variant}" \
    --output-json "${OUTPUT_ROOT}/cache-hash-${target}.json"

  "${PYTHON_BIN}" "${SCRIPT_DIR}/bridge_trace_1ep.py" preflight \
    --machine-name "${MACHINE_NAME}" \
    --target "${target}" \
    --variant "${variant}" \
  ...
  (
    ...
    "${PYTHON_BIN}" "${SCRIPT_DIR}/bridge_trace_1ep.py" train \
      --machine-name "${MACHINE_NAME}" \
      --target "${target}" \
      --variant "${variant}" \
  ...
  (
    export CUDA_VISIBLE_DEVICES="${GPU}"
    "${PYTHON_BIN}" "${SCRIPT_DIR}/bridge_trace_1ep.py" hash-probe \
      --machine-name "${MACHINE_NAME}" \
      --target "${target}" \
      --init-checkpoint "${INIT_CHECKPOINT}" \
