...
if [[ "$#" -lt 1 ]]; then
  cat >&2 <<'USAGE'
Usage:
  run_update_norm_event_trace_probe_queue.sh <queue-name>

Queues:
  event-2080ti-gpu0-baseline
  event-2080ti-gpu1-ucap0p5
  event-3090-gpu0-all
  event-3090-gpu0-baseline
  event-3090-gpu0-ucap0p5
USAGE
  exit 2
fi
...
TIMESTAMP="${UPDATE_NORM_EVENT_TRACE_TIMESTAMP:-$(date -u +%Y%m%dT%H%M%SZ)}"
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/update_norm_event_trace_probe.py" verify-init \
...
for target in "${TARGETS[@]}"; do
  ...
  "${PYTHON_BIN}" "${SCRIPT_DIR}/update_norm_event_trace_probe.py" cache-hash \
    --machine-name "${MACHINE_NAME}" \
    --target "${target}" \
    --variant "${variant}" \
    --output-json "${OUTPUT_ROOT}/cache-hash-${target}.json"

  "${PYTHON_BIN}" "${SCRIPT_DIR}/update_norm_event_trace_probe.py" preflight \
    --machine-name "${MACHINE_NAME}" \
    --target "${target}" \
    --variant "${variant}" \
  ...
  (
    ...
    "${PYTHON_BIN}" "${SCRIPT_DIR}/update_norm_event_trace_probe.py" train \
      --machine-name "${MACHINE_NAME}" \
      --target "${target}" \
      --variant "${variant}" \
  ...
  (
    export CUDA_VISIBLE_DEVICES="${GPU}"
    "${PYTHON_BIN}" "${SCRIPT_DIR}/update_norm_event_trace_probe.py" hash-probe \
      --machine-name "${MACHINE_NAME}" \
      --target "${target}" \
      --init-checkpoint "${INIT_CHECKPOINT}" \
