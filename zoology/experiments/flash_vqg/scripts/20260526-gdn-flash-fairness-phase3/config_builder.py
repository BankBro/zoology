...
PHASE3_DEFAULT_LAYOUTS = "banked-h2-b4-k256-v64-sharedv:2:4:256:64"
...
def _parse_phase3_layouts(raw: str) -> list[tuple[str, int, int, int, int]]:
    layouts: list[tuple[str, int, int, int, int]] = []
    seen: set[str] = set()
    for part in raw.split(","):
        ...
        if len(pieces) != 5:
            raise ValueError(
                "GDN_PHASE3_LAYOUTS 必须使用 label:num_heads:banks_per_head:bank_k_dim:bank_v_dim 格式, "
                f"当前收到: {value!r}"
            )
        label, num_heads_raw, banks_raw, bank_k_raw, bank_v_raw = pieces
        if not label:
            raise ValueError(f"GDN_PHASE3_LAYOUTS label 不能为空, 当前收到: {value!r}")
        if label in seen:
            continue
        layouts.append(
            (
                ...
                _parse_positive_int(banks_raw, field_name="banks_per_head"),
                _parse_positive_int(bank_k_raw, field_name="bank_k_dim"),
                _parse_positive_int(bank_v_raw, field_name="bank_v_dim"),
            )
        )
        seen.add(label)
    if not layouts:
        raise ValueError("GDN_PHASE3_LAYOUTS 不能为空.")
    return layouts


def _apply_banked_gdn_hparams(
    config,
    *,
    num_heads: int,
    banks_per_head: int,
    ...
    for mixer in mixer_configs:
        ...
        mixer["name"] = "zoology.mixers.gated_delta_net.GatedDeltaNetBankedK"
        mixer_kwargs.clear()
        mixer_kwargs.update(
            {
                "num_heads": num_heads,
                "banks_per_head": banks_per_head,
                "bank_k_dim": bank_k_dim,
                "bank_v_dim": bank_v_dim,
                "merge": "softmax_gate",
                "shared_v": True,
                "use_gate": use_gate,
    ...
    config.model.name = "gated_delta_net_banked_k"
    return config
...
def build_gdn_flash_fairness_phase3_configs(args):
    ...
    layouts = _parse_phase3_layouts(os.environ.get("GDN_PHASE3_LAYOUTS", PHASE3_DEFAULT_LAYOUTS))
    ...
    if explicit_run_id and (len(seed_values) > 1 or len(layouts) > 1):
        raise ValueError("--run-id 只能用于单个 seed 且单个 GDN_PHASE3_LAYOUTS 配置.")

    for base_config in base_gdn_configs:
        seed_value = int(base_config.seed)
        for label, num_heads, banks_per_head, bank_k_dim, bank_v_dim in layouts:
            config = _apply_banked_gdn_hparams(
                base_config,
                num_heads=num_heads,
                banks_per_head=banks_per_head,
                bank_k_dim=bank_k_dim,
