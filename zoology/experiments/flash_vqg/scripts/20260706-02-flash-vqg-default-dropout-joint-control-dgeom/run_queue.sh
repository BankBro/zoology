...
Queues:
  jointdgeom-2080ti-gpu0
  jointdgeom-3090-gpu0
USAGE
  exit 2
fi
...
if [[ -n "${JOINT_DGEOM_TARGETS:-}" ]]; then
  read -r -a TARGETS <<< "${JOINT_DGEOM_TARGETS}"
fi
...
run_one() {
  ...
  "${PYTHON_BIN}" "${SCRIPT_DIR}/joint_control_dgeom.py" cache-hash \
    --machine-name "${MACHINE_NAME}" \
    --target "${target}" \
    --variant "${variant}" \
    --output-json "${OUTPUT_ROOT}/cache-hash-${target}.json"

  "${PYTHON_BIN}" "${SCRIPT_DIR}/joint_control_dgeom.py" preflight \
    --machine-name "${MACHINE_NAME}" \
    --target "${target}" \
    --variant "${variant}" \
  ...
  (
    ...
    "${PYTHON_BIN}" "${SCRIPT_DIR}/joint_control_dgeom.py" train \
      --machine-name "${MACHINE_NAME}" \
      --target "${target}" \
      --variant "${variant}" \
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/joint_control_dgeom.py" verify-init \
  --machine-name "${MACHINE_NAME}" \
