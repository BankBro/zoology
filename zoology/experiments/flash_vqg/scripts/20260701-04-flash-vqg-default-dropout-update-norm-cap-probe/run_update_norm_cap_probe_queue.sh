...
if [[ "$#" -lt 1 ]]; then
  cat >&2 <<'USAGE'
Usage:
  run_update_norm_cap_probe_queue.sh <queue-name>

Queues:
  ucap-2080ti-gpu0-baseline-ucap0p25
  ucap-2080ti-gpu1-ucap0p5
  ucap-3090-gpu0-all
  ucap-3090-gpu0-baseline
  ucap-3090-gpu0-ucap0p5
  ucap-3090-gpu0-ucap0p25
USAGE
  exit 2
fi
...
TIMESTAMP="${UPDATE_NORM_CAP_PROBE_TIMESTAMP:-$(date -u +%Y%m%dT%H%M%SZ)}"
...
case "${QUEUE_NAME}" in
  ucap-2080ti-gpu0-baseline-ucap0p25)
    ...
    GPU="0"
    TARGETS=("baseline-r2" "ucap0p25-r2")
    ;;
  ucap-2080ti-gpu1-ucap0p5)
    MACHINE_NAME="${MACHINE_NAME:-2080ti}"
    GPU="1"
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/update_norm_cap_probe.py" verify-init \
...
for target in "${TARGETS[@]}"; do
  ...
  "${PYTHON_BIN}" "${SCRIPT_DIR}/update_norm_cap_probe.py" cache-hash \
    --machine-name "${MACHINE_NAME}" \
    --target "${target}" \
    --variant "${variant}" \
    --output-json "${OUTPUT_ROOT}/cache-hash-${target}.json"

  "${PYTHON_BIN}" "${SCRIPT_DIR}/update_norm_cap_probe.py" preflight \
    --machine-name "${MACHINE_NAME}" \
    --target "${target}" \
    --variant "${variant}" \
  ...
  (
    ...
    "${PYTHON_BIN}" "${SCRIPT_DIR}/update_norm_cap_probe.py" train \
      --machine-name "${MACHINE_NAME}" \
      --target "${target}" \
      --variant "${variant}" \
  ...
  (
    export CUDA_VISIBLE_DEVICES="${GPU}"
    "${PYTHON_BIN}" "${SCRIPT_DIR}/update_norm_cap_probe.py" hash-probe \
      --machine-name "${MACHINE_NAME}" \
      --target "${target}" \
      --init-checkpoint "${INIT_CHECKPOINT}" \
