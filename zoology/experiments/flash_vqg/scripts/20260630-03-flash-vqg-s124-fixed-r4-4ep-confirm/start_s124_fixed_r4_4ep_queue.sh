...
if [[ "$#" -lt 1 ]]; then
  cat >&2 <<'USAGE'
Usage:
  start_s124_fixed_r4_4ep_queue.sh <queue-name>
...
export OUTPUT_ROOT
export S124_FIXED_R4_4EP_TIMESTAMP="${TIMESTAMP}"

printf "%q " bash "${SCRIPT_DIR}/run_s124_fixed_r4_4ep_queue.sh" "${QUEUE_NAME}" > "${OUTPUT_ROOT}/command.txt"
printf "\n" >> "${OUTPUT_ROOT}/command.txt"

setsid bash "${SCRIPT_DIR}/run_s124_fixed_r4_4ep_queue.sh" "${QUEUE_NAME}" >"${QUEUE_LOG}" 2>&1 &
queue_pid="$!"
printf "%s\n" "${queue_pid}" > "${PID_FILE}"
printf "setsid\n" > "${SESSION_FILE}"
