...
if [[ "$#" -lt 1 ]]; then
  cat >&2 <<'USAGE'
Usage:
  start_repro_rerun_queue.sh <queue-name>
USAGE
  exit 2
fi
...
export OUTPUT_ROOT
export INJECTION_WARMUP_REPRO_RERUN_TIMESTAMP="${TIMESTAMP}"

printf "%q " bash "${SCRIPT_DIR}/run_repro_rerun_queue.sh" "${QUEUE_NAME}" > "${OUTPUT_ROOT}/command.txt"
printf "\n" >> "${OUTPUT_ROOT}/command.txt"

setsid bash "${SCRIPT_DIR}/run_repro_rerun_queue.sh" "${QUEUE_NAME}" >"${QUEUE_LOG}" 2>&1 &
queue_pid="$!"
printf "%s\n" "${queue_pid}" > "${PID_FILE}"
printf "setsid\n" > "${SESSION_FILE}"
