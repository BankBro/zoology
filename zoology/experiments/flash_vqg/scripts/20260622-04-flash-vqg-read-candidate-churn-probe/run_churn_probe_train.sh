...
if [ $# -lt 1 ]; then
  cat >&2 <<'USAGE'
Usage: run_churn_probe_train.sh <target> [extra_args...]

Targets:
  cb256r8-readk4-s123-churn
  cb256r8-readk2-s123-churn
  cb128r8-readk4-s125-churn

Example:
  GPU_ID=0 bash run_churn_probe_train.sh cb256r8-readk4-s123-churn
USAGE
  exit 2
fi
...
PROJECT="${PROJECT:-flash_vqg_read_candidate_churn_probe}"
...
LAUNCH_ID_PREFIX="flash-vqg-20260622-04-churn-${TARGET}"
...
exec "${PYTHON_BIN}" -m zoology.experiments.flash_vqg.run_flash_vqg_suite \
  ...
  --read-churn-probe-valid-batches "${READ_CHURN_PROBE_VALID_BATCHES:-0}" \
