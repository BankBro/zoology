...
from common import FLASH_ROOT, PYTHON, REPO_ROOT, VARIANTS, atomic_write_json, load_json, run_root, run_tag, utc_now
...
class Queue:
    ...
    def status(self, value: str, **extra) -> None:
        atomic_write_json(
            ...
            {"status": value, "phase": self.phase, "run_tag": run_tag(), "updated_at_utc": utc_now(), **extra},
        )
    ...
    def train(self, variant: str, phase: str) -> None:
        ...
        self.process(
            ...
            [str(PYTHON), str(EXPERIMENT_SCRIPT), "train", "--variant", variant, "--phase", phase],
        )
    ...
    def run(self) -> int:
        ...
        self.phase = "evaluate"
        code = self.process("evaluate", [str(PYTHON), str(EVALUATE_SCRIPT)], accepted=(0, 2))
...
def main() -> int:
    ...
    try:
        ...
        queue.status("failed", error=f"{type(exc).__name__}: {exc}", traceback=traceback.format_exc())
