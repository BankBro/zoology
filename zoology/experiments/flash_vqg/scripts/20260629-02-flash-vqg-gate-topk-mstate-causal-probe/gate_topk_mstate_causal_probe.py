...
TARGET = "gate-topk-mstate-causal-probe-s123-r1"
BASE_VARIANT = "baseline"
CONFIG_VARIANT = "no-dropout"
DEFAULT_TRACE_STEPS = "0,1,4,16"
...
def _variant_name(args: argparse.Namespace) -> str:
    value = str(getattr(args, "variant_name", "") or BASE_VARIANT).strip()
    if not value:
        return BASE_VARIANT
    if "/" in value or "\\" in value or value in {".", ".."}:
        raise ValueError("--variant-name must be a simple path component.")
    return value


def _variant_output_path(args: argparse.Namespace, name: str) -> Path:
    return SCRIPT_DIR / "outputs" / _variant_name(args) / args.machine_name / name


def _build_config(machine_name: str, max_optimizer_steps: int, variant_name: str = BASE_VARIANT):
    config = NO_DROPOUT.build_config(
        target=BASE_TARGET,
        machine_name=machine_name,
        variant=CONFIG_VARIANT,
        logger_backend="none",
        trace_output_dir=SCRIPT_DIR / "outputs/legacy-read-trace-disabled" / variant_name / machine_name,
        max_epochs=1,
        max_train_steps=max_optimizer_steps,
        max_validation_batches=1,
    )
    config.launch_id = f"fvqg-{EXPERIMENT_ID}-{machine_name}-{variant_name}-{TARGET}"
    config.run_id = f"{EXPERIMENT_ID}-{machine_name}-{variant_name}-{TARGET}"
...
def _apply_gate_overrides(config: Any, args: argparse.Namespace) -> dict[str, Any]:
    overrides: dict[str, Any] = {}
    if getattr(args, "fox_gate_logit_normalizer", None) is not None:
        value = int(args.fox_gate_logit_normalizer)
        _set_flash_vqg_kwarg(config, "fox_gate_logit_normalizer", value)
        overrides["fox_gate_logit_normalizer"] = value
    if getattr(args, "fox_gate_logf_compute_mode", None) is not None:
        value = str(args.fox_gate_logf_compute_mode)
        _set_flash_vqg_kwarg(config, "fox_gate_logf_compute_mode", value)
        overrides["fox_gate_logf_compute_mode"] = value
    if getattr(args, "fox_gate_logf_round_quantum", None) is not None:
        value = float(args.fox_gate_logf_round_quantum)
        _set_flash_vqg_kwarg(config, "fox_gate_logf_round_quantum", value)
        overrides["fox_gate_logf_round_quantum"] = value
    if getattr(args, "fox_gate_logf_constant_f", None) is not None:
...
def run_preflight(args: argparse.Namespace) -> int:
    variant_name = _variant_name(args)
    config = _build_config(
        args.machine_name,
        max_optimizer_steps=args.max_optimizer_steps,
        variant_name=variant_name,
    )
    config_overrides = _apply_gate_overrides(config, args)
    ...
    output = args.output_json or _variant_output_path(args, "preflight.json")
...
def run_probe(args: argparse.Namespace) -> int:
    ...
    output = args.output_json or _variant_output_path(args, "probe.json")
    payload = {
        "schema_version": 1,
        "experiment_id": EXPERIMENT_ID,
        "probe": {
...
def _trace_rank(row: dict[str, Any]) -> tuple[str, int, int, int, str]:
...
def _gate_comparison_rows(comparison_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    gate_names = {
        "fox_gate/input_x",
        "fox_gate/logits_cuda",
        "fox_gate/logf_cuda",
    ...
    return [row for row in comparison_rows if str(row.get("trace_name")) in gate_names]
    ...
    *,
    ...
    for row in rows:
        if (
            ...
            and str(row.get("optimizer_step")) == str(step)
            and str(row.get("trace_name")) == trace_name
        ):
            return str(row.get("sha256_match"))
    return ""


def _variant_summary_rows(comparison_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    variants = sorted({str(row.get("variant", "")) for row in comparison_rows})
    rows: list[dict[str, Any]] = []
    for variant in variants:
        variant_rows = [row for row in comparison_rows if str(row.get("variant")) == variant]
        mismatch_rows = [row for row in variant_rows if str(row.get("sha256_match")) == "False"]
        first = mismatch_rows[0] if mismatch_rows else {}
        rows.append(
            {
                "variant": variant,
                "comparison_row_count": len(variant_rows),
                "mismatch_row_count": len(mismatch_rows),
                "first_mismatch_step": first.get("optimizer_step", ""),
                "first_mismatch_micro_step": first.get("micro_step", ""),
                "first_mismatch_layer_idx": first.get("layer_idx", ""),
                "first_mismatch_trace_name": first.get("trace_name", ""),
                "step16_logf_match": _step_trace_match(
                    variant_rows,
                    variant=variant,
                    step=16,
                    trace_name="fox_gate/logf_cuda",
                ),
                "step16_m_state_match": _step_trace_match(
                    variant_rows,
                    variant=variant,
                    step=16,
                    trace_name="state_build/M_state",
                ),
                "step16_top_idx_match": _step_trace_match(
                    variant_rows,
                    variant=variant,
                    step=16,
                    trace_name="phase2_read/top_idx",
                ),
                "step16_loss_match": _step_trace_match(
                    variant_rows,
                    variant=variant,
                    step=16,
                    trace_name="forward/loss",
...
def run_collect(args: argparse.Namespace) -> int:
    ...
    gate_rows = _gate_comparison_rows(comparison_rows)
    variant_rows = _variant_summary_rows(comparison_rows)
    ...
    _write_csv(artifact_dir / "gate-comparison-summary.csv", gate_rows)
    ...
    metadata = {
        ...
        "gate_comparison_row_count": len(gate_rows),
    ...
    readme = (
        ...
        "This artifact contains the layer-1 Flash-VQG FoX gate/logf stability probe summaries.\n\n"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--variant-name", default=BASE_VARIANT)
    ...
    p.add_argument("--fox-gate-logit-normalizer", type=int)
    p.add_argument("--fox-gate-logf-compute-mode", choices=["default", "fp32_linear"])
    p.add_argument("--fox-gate-logf-round-quantum", type=float)
    p.add_argument("--fox-gate-logf-constant-f", type=float)
    p.add_argument("--fox-remote-read-topk", type=int)
    p.add_argument("--fox-gd-residual-residual-norm-mode", choices=["rmsnorm", "raw", "zero"])
