...
if [[ "$#" -lt 1 ]]; then
  cat >&2 <<'USAGE'
Usage:
  run_safe_limiter_queue.sh <queue-name>
...
SOURCE_SCRIPT_DIR="${REPO_ROOT}/zoology/experiments/flash_vqg/scripts/20260703-04-flash-vqg-safe-limiter-readk2-readk4-screen"
PYTHON_BIN="${PYTHON_BIN:-/home/lyj/miniconda3/envs/flash-vqg/bin/python}"
TIMESTAMP="${SAFE_LIMITER_TIMESTAMP:-$(date -u +%Y%m%dT%H%M%SZ)}"
...
if [[ -n "${SAFE_LIMITER_TARGETS:-}" ]]; then
  read -r -a TARGETS <<< "${SAFE_LIMITER_TARGETS}"
fi
...
"${PYTHON_BIN}" "${SOURCE_SCRIPT_DIR}/safe_limiter_readk_screen.py" verify-init \
...
for target in "${TARGETS[@]}"; do
  ...
  "${PYTHON_BIN}" "${SOURCE_SCRIPT_DIR}/safe_limiter_readk_screen.py" cache-hash \
    --machine-name "${MACHINE_NAME}" \
    --target "${target}" \
    --variant "${variant}" \
    --output-json "${OUTPUT_ROOT}/cache-hash-${target}.json"

  "${PYTHON_BIN}" "${SOURCE_SCRIPT_DIR}/safe_limiter_readk_screen.py" preflight \
    --machine-name "${MACHINE_NAME}" \
    --target "${target}" \
    --variant "${variant}" \
  ...
  "${PYTHON_BIN}" "${SOURCE_SCRIPT_DIR}/safe_limiter_batch_preflight.py" \
    --machine-name "${MACHINE_NAME}" \
    --target "${target}" \
    --variant "${variant}" \
    --max-epochs "${MAX_EPOCHS}" \
  ...
  (
    ...
    "${PYTHON_BIN}" "${SOURCE_SCRIPT_DIR}/safe_limiter_readk_screen.py" train \
      --machine-name "${MACHINE_NAME}" \
      --target "${target}" \
      --variant "${variant}" \
  ...
  if [[ "${train_status}" -eq 0 ]]; then
    ...
    echo "continuing queue after failed target=${target}" >&2
  fi
done
