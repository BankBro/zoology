...
def quality_summary(
    ...
    for seed in SEEDS:
        delta = standard[("a1-post-phase1", seed)] - standard[("a0-off", seed)]
        ...
        paired.append(
            {
                ...
                "a0": standard[("a0-off", seed)],
                "a1": standard[("a1-post-phase1", seed)],
                "a1_minus_a0": delta,
            }
        )
        for shape in EXTRAPOLATION_SHAPES:
            a0 = last_eval[("a0-off", seed, shape)]
            a1 = last_eval[("a1-post-phase1", seed, shape)]
            delta = a1 - a0
...
def system_rows(training: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    grouped = {}
    for variant in ("a0-off", "a1-post-phase1"):
...
def collect(output_dir: Path) -> dict[str, Any]:
    ...
    trajectory = load_json(run_root() / "trajectory" / "summary.json")
    ...
    atomic_write_json(output_dir / "trajectory-summary.json", trajectory)
    summary = {
        "experiment_id": EXPERIMENT_ID,
        "run_tag": run_tag(),
