...
if [[ "$#" -lt 1 ]]; then
  cat >&2 <<'USAGE'
Usage:
  run_no_dropout_4ep_queue.sh <queue-name>
...
case "${QUEUE_NAME}" in
  2080ti-gpu0)
    ...
    TARGETS=("no-dropout-4ep-s123-r1:0")
    ;;
  3090-gpu0)
    MACHINE_NAME="${MACHINE_NAME:-3090}"
    TARGETS=("no-dropout-4ep-s123-r1:0" "no-dropout-4ep-s123-r2:0")
    ;;
  *)
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/no_dropout_4ep.py" verify-init \
  --machine-name "${MACHINE_NAME}" \
  --checkpoint "${INIT_CHECKPOINT}" \
  --output-json "${OUTPUT_ROOT}/init-verify.json"

"${PYTHON_BIN}" "${SCRIPT_DIR}/no_dropout_4ep.py" cache-hash \
  --machine-name "${MACHINE_NAME}" \
  --target "no-dropout-4ep-s123-r1" \
  --output-json "${OUTPUT_ROOT}/cache-hash.json"

"${PYTHON_BIN}" "${SCRIPT_DIR}/no_dropout_4ep.py" preflight \
  --machine-name "${MACHINE_NAME}" \
  --target "no-dropout-4ep-s123-r1" \
...
for item in "${TARGETS[@]}"; do
  ...
  append_status "${QUEUE_NAME}" "${MACHINE_NAME}" "${target}" "no-dropout" "${gpu}" "" "pending" "${log_path}" "${config_json}" "${result_json}" "" ""
done
...
for item in "${TARGETS[@]}"; do
  ...
  (
    ...
    "${PYTHON_BIN}" "${SCRIPT_DIR}/no_dropout_4ep.py" train \
      --machine-name "${MACHINE_NAME}" \
      --target "${target}" \
      --variant "no-dropout" \
      ...
      --max-epochs 4 >"${log_path}" 2>&1
  ) &
  pid="$!"
  append_status "${QUEUE_NAME}" "${MACHINE_NAME}" "${target}" "no-dropout" "${gpu}" "${pid}" "started" "${log_path}" "${config_json}" "${result_json}" "${started_at}" ""
  ...
  if monitor_pid "${pid}" "${log_path}" "${target}"; then
    ...
    append_status "${QUEUE_NAME}" "${MACHINE_NAME}" "${target}" "no-dropout" "${gpu}" "${pid}" "completed" "${log_path}" "${config_json}" "${result_json}" "${started_at}" "${finished_at}"
  else
    status=$?
    finished_at="$(date -Iseconds)"
    append_status "${QUEUE_NAME}" "${MACHINE_NAME}" "${target}" "no-dropout" "${gpu}" "${pid}" "failed:${status}" "${log_path}" "${config_json}" "${result_json}" "${started_at}" "${finished_at}"
