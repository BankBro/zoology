...
if [[ "${SEED_VALUES}" == *,* ]]; then
  echo "decoupled baseline probe 只接受单个 SEED_VALUES, 当前收到: ${SEED_VALUES}" >&2
  exit 1
fi

LAUNCH_ID_PREFIX="${LAUNCH_ID_PREFIX:-flash-vqg-20260402-clr-v1-e2b-baseline-s${SEED_VALUES}-d${DATA_SEED}}"
BUILDER_SPEC="${SCRIPT_DIR}/config_builder.py:build_e2b_baseline_probe_decoupled_configs"
