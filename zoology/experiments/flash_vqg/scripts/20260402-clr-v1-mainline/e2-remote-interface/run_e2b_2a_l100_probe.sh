...
if [[ "${SEED_VALUES}" == *,* ]]; then
  echo "e2b-2a-l100 probe 只接受单个 SEED_VALUES, 当前收到: ${SEED_VALUES}" >&2
  exit 1
fi
if [[ "${SEED_VALUES}" != "${DATA_SEED}" ]]; then
  echo "e2b-2a-l100 probe 要求 SEED_VALUES 与 DATA_SEED 相同, 当前收到 SEED_VALUES=${SEED_VALUES}, DATA_SEED=${DATA_SEED}" >&2
  exit 1
fi

DEFAULT_LAUNCH_ID_PREFIX="flash-vqg-20260402-clr-v1-e2b-2a-l100-s${SEED_VALUES}"
if [[ "${SEED_VALUES}" == "123" ]]; then
  DEFAULT_LAUNCH_ID_PREFIX="${DEFAULT_LAUNCH_ID_PREFIX}-rerun"
fi
LAUNCH_ID_PREFIX="${LAUNCH_ID_PREFIX:-${DEFAULT_LAUNCH_ID_PREFIX}}"
BUILDER_SPEC="${SCRIPT_DIR}/config_builder.py:build_e2b_2a_l100_probe_configs"
