...
echo "==> Running E2b smoke on GPU ${GPU_ID}"
OUTPUT="$(
  "${PYTHON_BIN}" "${SCRIPT_DIR}/smoke_e2b_batch_accum.py" \
    --gpu "${GPU_ID}" \
    --backend "${BACKEND}" \
    --dmodels "${DMODEL}" \
    --learning-rates "${LR}" \
    ...
    --project "${PROJECT}" \
    --entity "${ENTITY}" \
    --launch-id-prefix "${LAUNCH_ID_PREFIX_E2B_SMOKE}" \
...
if [[ -z "${SMOKE_ENV_FILE}" || ! -f "${SMOKE_ENV_FILE}" ]]; then
  echo "E2b smoke 未产出可用的 SMOKE_ENV_FILE." >&2
  exit 1
fi

cp "${SMOKE_ENV_FILE}" "${SCRIPT_DIR}/e2b_smoke.env"
echo "E2b smoke 参数已写入 ${SCRIPT_DIR}/e2b_smoke.env"
