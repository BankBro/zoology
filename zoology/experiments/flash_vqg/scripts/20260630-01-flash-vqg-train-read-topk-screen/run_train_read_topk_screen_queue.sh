...
if [[ "$#" -lt 1 ]]; then
  cat >&2 <<'USAGE'
Usage:
  run_train_read_topk_screen_queue.sh <queue-name>
...
TIMESTAMP="${TRAIN_READ_TOPK_SCREEN_TIMESTAMP:-$(date -u +%Y%m%dT%H%M%SZ)}"
...
case "${QUEUE_NAME}" in
  2080ti-gpu0)
    ...
    TARGETS=("fixed-r2-baseline:fixed-r2-baseline:0" "sched64to4:sched64to4:0")
    ;;
  2080ti-gpu1)
    MACHINE_NAME="${MACHINE_NAME:-2080ti}"
    TARGETS=("fixed-r4:fixed-r4:1" "sched16to4:sched16to4:1")
    ;;
  3090-gpu0)
    MACHINE_NAME="${MACHINE_NAME:-3090}"
    TARGETS=(
      "fixed-r2-baseline:fixed-r2-baseline:0"
      "fixed-r4:fixed-r4:0"
      "sched16to4:sched16to4:0"
      "sched64to4:sched64to4:0"
    )
    ;;
  *)
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/train_read_topk_screen.py" verify-init \
  --machine-name "${MACHINE_NAME}" \
  --checkpoint "${INIT_CHECKPOINT}" \
  --output-json "${OUTPUT_ROOT}/init-verify.json"

"${PYTHON_BIN}" "${SCRIPT_DIR}/train_read_topk_screen.py" cache-hash \
  --machine-name "${MACHINE_NAME}" \
...
for item in "${TARGETS[@]}"; do
  ...
  rest="${item#*:}"
  variant="${rest%%:*}"
  "${PYTHON_BIN}" "${SCRIPT_DIR}/train_read_topk_screen.py" preflight \
    --machine-name "${MACHINE_NAME}" \
    --target "${target}" \
    --variant "${variant}" \
    --max-epochs 1 \
...
for item in "${TARGETS[@]}"; do
  ...
  (
    ...
    "${PYTHON_BIN}" "${SCRIPT_DIR}/train_read_topk_screen.py" train \
      --machine-name "${MACHINE_NAME}" \
      --target "${target}" \
      --variant "${variant}" \
