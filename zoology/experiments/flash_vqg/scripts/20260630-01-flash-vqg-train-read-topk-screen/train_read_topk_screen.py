...
EXPERIMENT_ID = "20260630-01-flash-vqg-train-read-topk-screen"
...
TARGETS = ("fixed-r2-baseline", "fixed-r4", "sched16to4", "sched64to4")
VARIANTS: dict[str, dict[str, Any]] = {
    ...
    "sched16to4": {
        "kind": "linear_int_schedule",
        "description": "linearly shrink train-time read_topk from 16 to 4 over train forward steps 0..448",
        "fox_remote_read_topk": None,
        "fox_remote_read_topk_initial": 16,
        "fox_remote_read_topk_final": 4,
        "fox_remote_read_topk_release_start_train_steps": 0,
        "fox_remote_read_topk_release_end_train_steps": 448,
        "fox_remote_read_topk_schedule": "linear_int",
        "fox_remote_read_topk_eval_policy": "scheduled",
        "fox_gd_residual_dense_read_chunked": False,
    },
    "sched64to4": {
        "kind": "linear_int_schedule",
        "description": "linearly shrink train-time read_topk from 64 to 4 over train forward steps 0..448",
        "fox_remote_read_topk": None,
        "fox_remote_read_topk_initial": 64,
        "fox_remote_read_topk_final": 4,
        "fox_remote_read_topk_release_start_train_steps": 0,
        "fox_remote_read_topk_release_end_train_steps": 448,
        "fox_remote_read_topk_schedule": "linear_int",
...
def _builder_read_topk_for_variant(variant: str) -> int:
    spec = _variant_config(variant)
    fixed = spec.get("fox_remote_read_topk")
    if fixed is not None:
        return int(fixed)
    final = spec.get("fox_remote_read_topk_final")
    if final is None:
        raise ValueError(f"Variant {variant} has neither fixed nor final read_topk.")
    return int(final)


def _apply_variant_to_config(config: Any, variant: str) -> None:
    spec = _variant_config(variant)
    for key in (
        ...
        _set_flash_vqg_kwarg(config, key, spec[key])
...
def _base_args(
    *,
    ...
    if target != variant:
        raise ValueError(f"Expected target and variant to match, got target={target} variant={variant}")
    builder_read_topk = _builder_read_topk_for_variant(variant)
    return Namespace(
        ...
        fox_remote_read_topk_values=str(builder_read_topk),
        ...
        project="flash_vqg_train_read_topk_screen",
        entity="scu-mclab",
        ...
        read_trace_train_steps="0,64,130,176,203,352,353,448,528,704",
        experiment_mode=f"{EXPERIMENT_ID}_{variant}_s123_d123_b64ga4_{machine_name}",
        run_id=f"{EXPERIMENT_ID}-{variant}-s123-d123-b64ga4-{machine_name}",
    )
    ...
    result = {
        ...
        **flash_settings,
        "passed": (
            ...
            and _variant_settings_match(flash_settings, args.variant)
        ),
    }
    if args.output_json:
...
def run_collect(args: argparse.Namespace) -> int:
    ...
    for status_path in sorted(outputs_dir.glob("*/queue-status.tsv")):
        ...
        for row in rows_by_key.values():
            ...
            variant_spec = result_payload.get("variant_spec") or VARIANTS.get(row.get("variant", ""), {})
            duration_seconds = ""
            ...
            run_rows.append(
                {
                    ...
                    "variant_kind": variant_spec.get("kind", ""),
                    ...
                    "fox_remote_read_topk_initial": _find_nested_key(
                        config_model, "fox_remote_read_topk_initial"
                    ),
                    "fox_remote_read_topk_final": _find_nested_key(
                        config_model, "fox_remote_read_topk_final"
                    ),
                    "fox_remote_read_topk_release_start_train_steps": _find_nested_key(
                        config_model, "fox_remote_read_topk_release_start_train_steps"
                    ),
                    "fox_remote_read_topk_release_end_train_steps": _find_nested_key(
                        config_model, "fox_remote_read_topk_release_end_train_steps"
                    ),
                    "fox_remote_read_topk_schedule": _find_nested_key(
                        config_model, "fox_remote_read_topk_schedule"
                    ),
                    "fox_remote_read_topk_eval_policy": _find_nested_key(
                        config_model, "fox_remote_read_topk_eval_policy"
                    ),
    ...
    for variant in TARGETS:
        rows = [row for row in run_rows if row.get("variant") == variant]
        by_machine = {str(row.get("machine", "")): row for row in rows}
        row_2080 = by_machine.get("2080ti")
        row_3090 = by_machine.get("3090")
        final_2080 = (
            float(row_2080["final_1024x256_accuracy"])
            if row_2080 and row_2080.get("final_1024x256_accuracy") not in ("", None)
            else None
        )
        final_3090 = (
            float(row_3090["final_1024x256_accuracy"])
            if row_3090 and row_3090.get("final_1024x256_accuracy") not in ("", None)
            else None
        )
        best_2080 = (
            float(row_2080["best_1024x256_accuracy"])
            if row_2080 and row_2080.get("best_1024x256_accuracy") not in ("", None)
            else None
        )
        best_3090 = (
            float(row_3090["best_1024x256_accuracy"])
            if row_3090 and row_3090.get("best_1024x256_accuracy") not in ("", None)
            else None
        )
        final_gap = (
            abs(final_2080 - final_3090)
            if final_2080 is not None and final_3090 is not None
            else None
        )
        best_gap = (
            abs(best_2080 - best_3090)
            if best_2080 is not None and best_3090 is not None
            else None
        )
        summary = {
            "variant": variant,
            "variant_kind": VARIANTS[variant]["kind"],
            "description": VARIANTS[variant]["description"],
            "completed_machines": ",".join(sorted(by_machine)),
            "completed_count": len(rows),
            "final_1024x256_2080ti": "" if final_2080 is None else final_2080,
            "final_1024x256_3090": "" if final_3090 is None else final_3090,
            "final_gap": "" if final_gap is None else final_gap,
            "final_gap_percentage_points": "" if final_gap is None else final_gap * 100.0,
            "final_within_4pp": "" if final_gap is None else final_gap <= 0.04,
            "best_1024x256_2080ti": "" if best_2080 is None else best_2080,
            "best_1024x256_3090": "" if best_3090 is None else best_3090,
            "best_gap": "" if best_gap is None else best_gap,
            "best_gap_percentage_points": "" if best_gap is None else best_gap * 100.0,
            "best_within_4pp": "" if best_gap is None else best_gap <= 0.04,
            "fox_remote_read_topk": VARIANTS[variant]["fox_remote_read_topk"],
            "fox_remote_read_topk_initial": VARIANTS[variant]["fox_remote_read_topk_initial"],
            "fox_remote_read_topk_final": VARIANTS[variant]["fox_remote_read_topk_final"],
            "release_start": VARIANTS[variant]["fox_remote_read_topk_release_start_train_steps"],
            "release_end": VARIANTS[variant]["fox_remote_read_topk_release_end_train_steps"],
            "schedule": VARIANTS[variant]["fox_remote_read_topk_schedule"],
            "dense_read_chunked": VARIANTS[variant]["fox_gd_residual_dense_read_chunked"],
        }
        ...
        if final_gap is not None:
            comparison_rows.append(
                {
                    "variant": variant,
                    "reference_machine": "2080ti",
                    "reference_1024x256_accuracy": final_2080,
                    "candidate_machine": "3090",
                    "candidate_1024x256_accuracy": final_3090,
                    "absolute_gap": final_gap,
                    "gap_percentage_points": final_gap * 100.0,
                    "within_4pp": final_gap <= 0.04,
                }
            )
    ...
    _write_csv(artifact_dir / "variant-summary.csv", variant_rows)
    ...
    readme = (
        ...
        "共同配置: `seed=123`, `data_seed=123`, `cb64-r16`, `write_topk=4`, "
        "no-dropout, canonical cache/init, 1 epoch. 变量只有 train-time read_topk 策略.\n\n"
        "Variants: `fixed-r2-baseline`, `fixed-r4`, `sched16to4`, `sched64to4`. "
        "`sched*to4` 使用 `linear_int` 在 train forward step `0..448` 逐步收缩到 4.\n\n"
        "## 文件\n\n"
