...
if [[ "$#" -lt 1 ]]; then
  cat >&2 <<'USAGE'
Usage:
  run_default_dropout_fixed_r4_1ep_queue.sh <queue-name>
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/default_dropout_fixed_r4_1ep.py" verify-init \
  --machine-name "${MACHINE_NAME}" \
  --checkpoint "${INIT_CHECKPOINT}" \
  --output-json "${OUTPUT_ROOT}/init-verify.json"

"${PYTHON_BIN}" "${SCRIPT_DIR}/default_dropout_fixed_r4_1ep.py" cache-hash \
  --machine-name "${MACHINE_NAME}" \
  --target "${TARGET}" \
  --variant "${VARIANT}" \
  --output-json "${OUTPUT_ROOT}/cache-hash.json"

"${PYTHON_BIN}" "${SCRIPT_DIR}/default_dropout_fixed_r4_1ep.py" preflight \
  --machine-name "${MACHINE_NAME}" \
  --target "${TARGET}" \
  --variant "${VARIANT}" \
...
(
  ...
  "${PYTHON_BIN}" "${SCRIPT_DIR}/default_dropout_fixed_r4_1ep.py" train \
    --machine-name "${MACHINE_NAME}" \
    --target "${TARGET}" \
    --variant "${VARIANT}" \
